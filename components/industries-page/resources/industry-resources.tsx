...
export const IndustryResource = () => {
    const industryPageData: any = useSelector((state: any) => state?.industry?.industry_page_Data)
    ...
    return (
        <>
            {industryPageData?.section_resources && <Container dimension="extra-large" className="home-blog">
                <Stack as="div" direction="column" rowGap={64} hPadding={24}>
                    <SectionHead

                        title={industryPageData?.section_resources?.main_title}
                        underneethSmallTitle={industryPageData?.section_resources?.sub_title}
                    />
                    <Stack as="div" className="mt-16 tab--content--block animate__animated animate__fadeInUp" direction="row" wrap rowGap={32} columnGap={32} hAlign="center">
                        {industryPageData?.section_resources?.reference?.length > 0 &&
                            industryPageData?.section_resources?.reference?.map((item: any, index: number) => {
