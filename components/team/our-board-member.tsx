...
export const OurBoardMember = () => {
    const { board_members }: any = useSelector((state: any) => state?.team)
    return (
        <>
            {board_members && <Container dimension="extra-large">
                <Stack as="div" hPadding={24} rowGap={64}>
                    <SectionHead
                        title={board_members?.title}
                        headText={board_members?.sub_title}
                    />
                    {board_members?.members?.length > 0 && <Stack className="mt-16 card--sec" direction="row" wrap columnGap={32} rowGap={32}>
                        {board_members?.members?.map((item: any) => {
                            return (
                                <ImageCard
